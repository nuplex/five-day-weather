...
For a small project, this level of organization isn't necessary, however it's always good to keep
...
visual components later. Look and feel updates we're incremental, and most was saved for the end. Each component and 
feature was added iteratively, starting from the basic MVP of an input field, ending with a full application.
9. Discussed below were the in-process trade-offs. Determining when to shelve something or to keep working on it is a 
constant decision.

#### Design Process

My goal was to give the app a minimalist feel, while making it easy to navigate (some minimalist designs are not, 
ironically). I placed emphasis on keeping it clean and responsive, though de-emphasized smooth transition animations, 
...
adds a background (and margins) to all days, so they are easily distinguishable on smaller screen.
...
zip code, and lat-long is an adequate feature. The application using a minimalist design instead of barebones HTML is
...
the closest city to you. It would be better to have this fill into the bar **first**. Currently the bar fills after it's 
already looked up your forecast. This feature could be done by first matching lat/long with a city in 
the OWM bulk download of cities, and returning that city.

### View Weather Details

By clicking 'View Details' on a day, the app can display more weather data for that day.

#### Wanted Enhancement - View Details in Modal

Initially I was going to display details in a card above or below the forecast, however, on a smaller screen, this would
mean after clicking `Show Details` the user would have to scroll to see the details. A modal would be best. I scrapped 
modals as a trade-off.
...
- Determine if current `!important`'s can be replaced with alternative CSS
...
I do not know the steps to consistently reproduce this error. It is seemingly random when it occurs. The best coordinates to test with are those right
